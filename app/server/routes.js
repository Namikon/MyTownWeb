...
var MYSQL = require('./modules/mysql-manager');
var SQLCONST = require('./mysql_queries');
    ...
    */
    ...
    app.get('/towns', function(req, res) {
        if (req.session.user == null){
            ...
            var townList = [];
            var useQuery = SQLCONST.SQL_GET_TOWN_LIST_MEMBER;

            if (req.session.user.staff == 1)
                useQuery = SQLCONST.SQL_GET_TOWN_LIST_STAFF;

            connection.query(useQuery, function(err, rows, fields)
            {
                if (err) {
                    console.log(err);
